...
// --- CONSTANTS ---
// Rotating words now integrated directly into the headline
...
export default function HomePage() {
  ...
  // State definitions (Logic unchanged)
  ...
  // Auth + profile load (Logic unchanged)
  useEffect(() => {
  ...
  // Rotate words ticker (Logic unchanged)
  useEffect(() => {
  ...
  // BAE CTA behavior (Logic unchanged)
  ...
  return (
    ...
    <main className="relative min-h-screen overflow-hidden bg-black text-white">
      ...
      {/* Header (Simplified to just the title) */}
      <header className="fixed top-0 inset-x-0 z-20 flex items-center justify-between px-6 h-[72px] backdrop-blur-md bg-black/50 border-b border-fuchsia-500/20">
        ...
        {/* Removed Sign Out/Profile links from the main view to reduce clutter */}
      ...
      <section className="relative flex flex-col items-center justify-center text-center px-6 min-h-screen z-10">
        ...
        {/* 1. HUGE, BRIGHT, SHINING TAGLINE */}
        <motion.h2
          initial={{ opacity: 0, y: 30 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8 }}
          // Massive font size (9xl) and an aggressive text shadow for the bright shine
          className="text-7xl sm:text-8xl lg:text-[10rem] font-extrabold mb-12 drop-shadow-[0_0_50px_rgba(255,160,255,1)] leading-none" 
        >
          Meet. Match.
          {/* BAE Word: Bold, high-contrast glow */}
          <span className="bg-gradient-to-r from-yellow-300 to-pink-400 bg-clip-text text-transparent ml-4">
            BAE.
          </span>
        </motion.h2>

        {/* 2. Rotational Words - MOVED INTO A BOLD TAGLINE */}
        <motion.p
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.2, duration: 0.8 }}
          className="text-3xl sm:text-4xl font-medium mb-16 text-white/95 drop-shadow-lg"
        >
          ...
          <span className="inline-block min-w-[12rem] relative align-baseline">
            <AnimatePresence mode="wait">
              <motion.span
                ...
                // Rotating word is super bright, white/pink focal point
        ...
        {/* CTA Button: Sleek, high-energy, and inviting */}
        <motion.button
          ...
          style={{
            background: "linear-gradient(90deg, #FF6F91, #FF9B85)", // High-energy pink/coral gradient
