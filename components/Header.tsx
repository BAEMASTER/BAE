...
export default function Header() {
  ...
  const doSignIn = () => router.push('/auth');
  ...
  return (
    ...
    >
      ...
      {!user && (
        <div className="w-full flex items-center justify-between">
          ...
          <button
            onClick={doSignIn}
            disabled={busy}
            className="px-6 py-2 rounded-full text-xl font-extrabold text-white
      ...
      {/* LOGGED IN HEADER (full options restored) */}
      {user && (
        <div className="mx-auto flex max-w-6xl w-full items-center gap-6 text-fuchsia-700 leading-normal">
          ...
          <nav className="flex items-center gap-6 text-lg font-semibold text-fuchsia-700/90">
            <Link href="/me" className="hover:text-fuchsia-800">Profile</Link>
            <Link href="/interests" className="hover:text-fuchsia-800">Interests</Link>
            <Link href="/devcheck" className="hover:text-fuchsia-800">DevCheck</Link>
          </nav>
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            <button
              onClick={doSignOut}
              disabled={busy}
              className="px-4 py-2 rounded-xl border border-white/40 text-base font-semibold
